...
- (void)creatRateLabel
{
    _rateNameLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, 0, self.width - 5, 12)];
    _rateNameLabel.text = @"收益率:";
    _rateNameLabel.textColor = GITCOLOR_TEXT_BLUE;
    _rateNameLabel.font = [UIFont systemFontOfSize:10];
    [self addSubview:_rateNameLabel];
    
    _rateLabel = [[UILabel alloc]initWithFrame:CGRectMake(_rateNameLabel.origin.x, _rateNameLabel.screenViewY, self.width, 12)];
    _rateLabel.text = [NSString stringWithFormat:@"%.2f%%",self.rate];
    _rateLabel.textColor = [UIColor colorWithString:@"#12b7f5"];
